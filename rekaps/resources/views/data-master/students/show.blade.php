...
<div class="p-2 grid items-center rounded-lg gap-3">
    <div class="justify-between">
        <h1 class="text-2xl lg:text-3xl font-semibold">
            <span style="border-bottom: 1px solid #B6BBC4;"><span>Data Siswa</span></span>
        </h1>
    </div>
    <div class="text-right">
        @if (Auth::user()->role == "admin")
        <a href="{{ route('students.create') }}" class="text-white p-2 rounded-lg" style="background-color: #0766AD;">
            Tambah Data
        </a>
        @endif
    </div>
                ...
                <tbody>
                    @forelse ($students as $student)
                        <tr>
                            ...
                            <td class="border-b border-gray-400 px-4 py-2 text-center">{{ $student->nis }}</td>
                            <td class="border-b border-gray-400 px-4 py-2 text-center">{{ $student->name }}</td>
                            <td class="border-b border-gray-400 px-4 py-2 text-center">{{ $student->rombel->rombel ?? 'N/A' }}</td>
                            <td class="border-b border-gray-400 px-4 py-2 text-center">{{ $student->rayon->rayon ?? 'N/A' }}</td>
                            @if (Auth::user()->role == "admin")
                                <td class="border-b border-gray-400 px-4 py-2 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('students.edit', $student->id) }}" class="rounded-lg p-2 text-white flex items-center" style="background-color: #0766AD;">
